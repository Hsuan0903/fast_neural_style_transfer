...
CONTENT_WEIGHT = 1e0
STYLE_WEIGHT = 1e6
CHKPT_FREQ = 0 # set to 0 to disable checkpoints
TEST_FREQ = 500 # set to 0 to disable test images

TEST_IMG_DIR = os.path.join("images/1-input", TEST_IMG) + ".jpg"
STYLE_IMG_DIR = os.path.join("images/2-style", STYLE_IMG) + ".jpg"
MODEL_SAVE_FOLDER = os.path.join(MODEL_FOLDER, STYLE_IMG)
TEST_SAVE_FOLDER = os.path.join(MODEL_FOLDER, STYLE_IMG, "test images")
...
def train():
    ...
    if TEST_FREQ != 0:
        ...
        test_image = utils.load_image(TEST_IMG_DIR)
    ...
    if CONTINUE is not None:
        state_dict = torch.load(os.path.join(MODEL_FOLDER, STYLE_IMG, CONTINUE))
    ...
    style = utils.load_image(STYLE_IMG_DIR, scale=STYLE_SCALE)
    ...
    model_filename = f"{STYLE_IMG}_epoch_{EPOCHS}_cw_{CONTENT_WEIGHT}_sw_{STYLE_WEIGHT}.pth"
